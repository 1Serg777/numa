...
namespace numa {
	...
	Vec2 RandomInSquare(double side) {
		return RandomInUnitSquare() * static_cast<float>(side); // TEST cast
	}
	...
	Vec3 RandomInCube(double side) {
		return RandomInUnitCube() * static_cast<float>(side); // TEST cast
	}
	...
	Vec3 RandomInSphere(double radius) {
		return RandomInUnitSphere() * static_cast<float>(radius); // TEST cast
	}
